...
env({ command: 'ls' }, { checkout: false }).then(env => {
  t.same(env.BUILDKITE_AGENT_NAME, 'agent')
  t.same(env.BUILDKITE_BUILD_PATH, path.join(os.homedir(), '.bksr', 'builds'))
  t.same(env.BUILDKITE_PLUGINS_PATH, path.join(os.homedir(), '.bksr', 'plugins'))
...
env({ command: 'ls' }, { checkout: true }).then(env => {
  t.same(env.BUILDKITE_HOOKS_PATH, undefined)
})

env({ commands: [ 'echo 1', 'echo 2' ] }, { checkout: false }).then(env => {
  t.same(env.BUILDKITE_COMMAND, 'echo 1\necho 2')
})

env({ plugins: { 'docker-compose': { 'run': 'echo' } } }, { checkout: false }).then(env => {
...
env({ command: 'ls', plugins: { 'docker-compose': { 'run': 'echo' } } }, { checkout: false }).then(env => {
  t.same(env.BUILDKITE_COMMAND, 'ls')
  t.same(env.BUILDKITE_PLUGINS, '[{"github.com/buildkite-plugins/docker-compose-buildkite-plugin":{"run":"echo"}}]')
})
